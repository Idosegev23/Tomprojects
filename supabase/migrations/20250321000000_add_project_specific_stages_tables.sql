-- ====================================================================
-- מיגרציה להוספת טבלת שלבים ייחודית לכל פרויקט
-- ====================================================================
...
-- ================================================================
-- פונקציה לסנכרון כל הטבלאות הייעודיות כאשר פרויקט נוצר
-- ================================================================
CREATE OR REPLACE FUNCTION sync_all_project_tables()
RETURNS void AS $$
DECLARE
  project_rec record;
BEGIN
  FOR project_rec IN SELECT id FROM projects LOOP
    -- 1. יצירת/סנכרון טבלת המשימות הייחודית
    BEGIN
      PERFORM create_project_table(project_rec.id);
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Error syncing tasks table for project %: %', project_rec.id, SQLERRM;
    END;
    
    -- 2. יצירת/סנכרון טבלת השלבים הייחודית
    BEGIN
      PERFORM sync_project_stages(project_rec.id);
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Error syncing stages table for project %: %', project_rec.id, SQLERRM;
    END;
  END LOOP;
END;
$$ LANGUAGE plpgsql; 