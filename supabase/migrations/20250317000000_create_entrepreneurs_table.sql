...
-- הוספת הרשאות לטבלת היזמים
ALTER TABLE entrepreneurs ENABLE ROW LEVEL SECURITY;

-- מדיניות הרשאות לטבלת היזמים
CREATE POLICY "Enable read access for all users" ON entrepreneurs
  FOR SELECT USING (true);

CREATE POLICY "Enable insert for authenticated users only" ON entrepreneurs
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Enable update for authenticated users only" ON entrepreneurs
  FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Enable delete for authenticated users only" ON entrepreneurs
