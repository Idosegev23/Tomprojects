-- פתרון סופי לבעיית אתחול טבלאות פרויקט
-- מבוסס על המבנה המדויק של הטבלאות
...
-- פונקציה ליצירת טבלת שלבים לפרויקט - התואמת למבנה הטבלה המקורי
...
-- פונקציה ליצירת טבלת משימות לפרויקט - התואמת למבנה הטבלה המקורי
...
-- פונקציה להעתקת שלבים לטבלת פרויקט - מותאמת למבנה הנכון
...
BEGIN
  ...
  -- העתקת שלבים מהטבלה הראשית - התאמה למבנה המדויק של טבלת stages
  FOR stage_rec IN 
    SELECT 
      s.id, s.title, s.hierarchical_number, s.due_date, s.status, s.progress, 
      s.color, s.parent_stage_id, s.dependencies, s.sort_order, 
      s.created_at, s.updated_at, project_id_param AS project_id
    FROM 
      stages s
    WHERE 
      s.project_id = project_id_param OR s.project_id IS NULL
  LOOP
    -- העתקת השלב לטבלת הפרויקט
    BEGIN
      EXECUTE format('
        INSERT INTO %I (
  ...
  -- יצירת שלב ברירת מחדל אם אין שלבים
  EXECUTE format('
    INSERT INTO %I (
...
-- פונקציה להעתקת משימה לטבלת פרויקט - מותאמת למבנה הנכון
...
-- פונקציה לאתחול טבלאות הפרויקט עם טיפול בשגיאות
...
BEGIN
  ...
  -- 1. יצירת טבלאות הפרויקט
  PERFORM create_project_table(project_id_param);
  PERFORM create_project_stages_table(project_id_param);
  
  -- 2. העתקת שלבים (אם ביקשו)
  IF create_default_stages THEN
    BEGIN
      PERFORM copy_stages_to_project_table(project_id_param);
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Error copying stages for project %: %', project_id_param, SQLERRM;
    END;
  END IF;
  
  -- 3. טיפול במשימות (אם ביקשו)
  IF create_default_tasks THEN
    -- העתקת משימות ספציפיות אם ציינו
