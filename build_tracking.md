...
### 2025-04-01 - הוספת אנליטיקס של Vercel לכל דפי האפליקציה

- הוספת קומפוננטת `Analytics` מחבילת `@vercel/analytics/react` לקובץ `src/app/layout.tsx`
- קומפוננטת האנליטיקס הוטמעה בכל דפי האפליקציה באמצעות קובץ ה-layout הראשי
- השינוי מאפשר מעקב אחרי תנועת המשתמשים והתנהגותם באפליקציה
- המידע הנאסף יכול לסייע בקבלת החלטות לגבי שיפורים ועדכונים עתידיים
...
## 2025-07-24
### תיקון שגיאות בסנכרון שלבים וטבלאות ייחודיות לפרויקט

**בעיה:**  
שגיאות בעת יצירת וקריאת שלבים מטבלאות פרויקט ייחודיות, עם הודעות שגיאה כמו `"Could not find the 'description' column of 'project_XX_stages' in the schema cache"` ו-`"relation "public.information_schema.tables" does not exist"`.

**פתרון:**  
1. עדכון פונקציית `createStage` בשירות השלבים כדי שתתאים למבנה העמודות הקיים בטבלת השלבים:
   - הוספת שדה `description` כנדרש בטבלה
   - שימוש ב-`sort_order` במקום `order`
   - התאמת כל השדות למבנה העמודות המדויק של הטבלה

2. שינוי אופן הבדיקה של טבלה קיימת בפונקציית `checkIfTableExists`:
   - הימנעות משימוש בשאילתת `information_schema.tables` שלא נתמכת
   - ביצוע שאילתה ישירה על הטבלה וניתוח השגיאות לזיהוי אם הטבלה קיימת

3. שיפור הטיפול בשגיאות והוספת לוגים מפורטים כדי לעקוב אחר מקור הבעיות.

**יתרונות:**
- מניעת שגיאות מסוג "column does not exist in schema cache"
- הבטחת עבודה תקינה של סנכרון שלבים בין הטבלה הכללית לטבלאות ייחודיות של פרויקטים
- שיפור יצירת שלבים חדשים בטבלאות הייחודיות
- טיפול מדויק יותר במבנה הטבלאות

**קבצים ששונו:**
- `src/lib/services/stageService.ts`: עדכון פונקציות `createStage`, `checkIfTableExists`

## 2025-07-25 - תיקון תקלות ביצירה וסנכרון של שלבים עבור פרויקטים

**בעיה:**
- שגיאות טיפוס בקובץ `stageService.ts` כאשר מנסים לגשת לשדות שלא קיימים בממשק הבסיסי של `Stage` בזמן יצירת שלבים חדשים
- שגיאה בעת קריאה לטבלה `information_schema.tables` שלא קיימת בסביבת הפיתוח
- התנגשות בין הכותרות של השדות: `order` לעומת `sort_order`

**פתרון:**
1. עדכון פונקציית `createStage` להשתמש בטיפוס `any` ולהגדיר את כל השדות הדרושים באופן מפורש
2. הוספת בדיקה אם שדות קיימים באובייקט השלב לפני השימוש בהם
3. עדכון פונקציית `createDefaultStages` להגדיר את כל השדות הדרושים בשלבי ברירת המחדל
4. שינוי פונקציית `checkIfTableExists` לשימוש בשיטה פשוטה יותר שאינה דורשת גישה ל-`information_schema`
5. מיפוי משלב של השדה `order` לשדה `sort_order` בעת היצירה

**יתרונות:**
- מניעת שגיאות טיפוס ב-TypeScript
- תמיכה טובה יותר בסביבות בהן אין גישה ל-`information_schema`
- שלבים חדשים נוצרים עם כל השדות הדרושים מראש
- שיפור היציבות של סנכרון השלבים בין הטבלאות הכלליות לטבלאות הספציפיות של הפרויקט
...
## 2025-07-25 - תיקון בעיות עם בדיקת קיום טבלאות שלבים בפרויקטים חדשים

### בעיות שזוהו:
1. פונקציית `checkIfTableExists` ב-JavaScript מדווחת שגיאות 400 כאילו הטבלה קיימת.
2. בעיות נגישות ל-PostgreSQL דרך Supabase גורמות לכך שבדיקות קיום טבלה אינן מדויקות.
3. אי-התאמה בין בדיקות קיום טבלה בפונקציות JavaScript ו-SQL מובילה לתוצאות שגויות.

### הפתרונות שיושמו:
1. **שיפור פונקציית בדיקת קיום טבלה בצד קליינט:**
   - שונתה הפונקציה `checkIfTableExists` להשתמש בפונקציית RPC במקום ניסיון גישה ישיר.
   - נוספה בדיקה ספציפית לשגיאה 400 כדי לוודא שהטבלה לא קיימת במקרה זה.
   - נוסף מנגנון גיבוי למקרה שפונקציית ה-RPC נכשלת.

2. **שיפור בדיקת קיום טבלאות בקוד ה-API:**
   - עודכנה הבדיקה ב-`src/app/api/projects/sync-stages/route.ts` לשימוש בפונקציית RPC.
   - שופרו הודעות השגיאה והלוגים לאיתור בעיות ביתר קלות.

### יתרונות הפתרון:
- מניעת דיווחים שגויים על קיום טבלאות.
- שיפור יכולת המעקב והניפוי שגיאות בלוגים.
- הפחתת מספר הניסיונות הכושלים לגשת לטבלאות לא קיימות.

### קבצים ששונו:
- `src/lib/services/stageService.ts`: עדכון פונקציית `checkIfTableExists`
- `src/app/api/projects/sync-stages/route.ts`: שיפור בדיקת קיום טבלאות

### המלצות להמשך:
יש לוודא שפונקציית RPC `check_table_exists` קיימת ונגישה בכל הסביבות. במקרה של העדר הפונקציה, המערכת תחזור למנגנון חלופי פחות יעיל.

## 2025-07-27 תיקון בעיית העתקת שלבים לטבלאות פרויקט ספציפיות

### בעיה
השלבים לא הועתקו כראוי מהטבלה הכללית לטבלה הספציפית של הפרויקט. בעוד שהמשימות הועתקו בהצלחה, השלבים נשארו בטבלה הכללית, מה שגרם לבעיות בתצוגה ובקישור בין משימות לשלבים.

### פתרון
1. נוספה פונקציה חדשה `syncStagesToProjectTable` בשירות `stageService` שמטפלת בכל תהליך העתקת השלבים:
   - בדיקה אם טבלת השלבים הספציפית קיימת, ויצירתה אם לא
   - שליפת השלבים מהטבלה הכללית (כולל שלבים כלליים וספציפיים לפרויקט)
   - העתקת כל השלבים לטבלה הספציפית תוך התאמת שדה project_id
   - טיפול בשגיאות ומצבי קצה כמו אי-קיום טבלאות או שלבים

2. שודרג נתיב ה-API `src/app/api/projects/sync-stages/route.ts` להשתמש בפונקציה החדשה:
   - שינוי הלוגיקה כך שהיא משתמשת בפונקציה החדשה בתור ברירת מחדל
   - שמירה על מנגנוני גיבוי במקרה של כשל (קריאה ישירה ל-RPC, בדיקת שלבים קיימים)
   - שיפור הטיפול בשגיאות והתיעוד

**יתרונות**:
- תהליך העתקת השלבים אמין יותר ומטפל בתרחישים שונים
- רמת שגיאות נמוכה יותר בתהליך הסנכרון
- שיפור בקישור בין משימות לשלבים בטבלאות הספציפיות של הפרויקט
- תמיכה בשלבים כלליים ובשלבים ספציפיים לפרויקט

**מצב נוכחי**:
- כעת השלבים מועתקים כראוי לטבלת הפרויקט הספציפית
- המשימות מקושרות לשלבים המתאימים בטבלת הפרויקט הספציפית
- תצוגת השלבים והמשימות מתפקדת כראוי בממשק המשתמש

## תיקון סנכרון שלבים בפרויקט ספציפי (2024-06-12)

**בעיה:**
שלבים לא הועתקו נכון מהטבלה הכללית לטבלה הספציפית של פרויקט, וכתוצאה מכך המשימות לא היו מקושרות לשלבים הנכונים בתצוגת המשתמש.

**פתרון:**
1. שכתוב מלא של נתיב ה-API `sync-stages` כך שהוא:
   - מזהה את השלבים המקושרים למשימות בפרויקט
   - מעתיק את השלבים הללו מהטבלה הכללית לטבלה הספציפית של הפרויקט
   - מעדכן את המשימות כך שיצביעו לשלב הנכון בטבלה הספציפית

2. יישום מנגנון חמש-שלבי:
   - שלב 1: איתור משימות עם מזהי שלבים
   - שלב 2: איסוף מזהי שלבים ייחודיים
   - שלב 3: שליפת מידע על השלבים מהטבלה הכללית
   - שלב 4: העתקת השלבים לטבלה הספציפית של הפרויקט
   - שלב 5: עדכון מזהי השלבים במשימות להצביע לשלבים החדשים

**יתרונות:**
- קישור נכון בין משימות לשלבים
- שיפור בתצוגת המשתמש של משימות והשלבים שלהן
- סנכרון אמין יותר בין הטבלה הכללית לטבלאות הספציפיות
- ניטור ורישום מפורט של תהליך הסנכרון

**סטטוס:**
✅ הפתרון נבדק והוא פועל כראוי.
✅ הסנכרון מזהה בהצלחה את השלבים הדרושים ומעתיק אותם לטבלה הספציפית.
✅ המשימות מעודכנות להצביע לשלבים בטבלה הספציפית.
✅ הפונקציות הבאות נמצאו פעילות במסד הנתונים: `check_table_exists`, `check_stages_table_exists`, `create_project_stages_table`, `copy_stages_to_project`, `sync_stages_on_change`.

## 2024-06-16: פתרון בעיית העמודה 'hierarchical_number' בטבלאות שלבים

### הבעיה

זוהתה בעיה בעת יצירת שלבים חדשים בפרויקטים מסוימים, כאשר ניסיון ליצור שלב חדש הוביל לשגיאה:

```
שגיאה ביצירת שלב חדש: {code: 'PGRST204', details: null, hint: null, message: "Could not find the 'hierarchical_number' column of project_XXX_stages in the schema cache"}
```

הבעיה נבעה מכך שבחלק מהפרויקטים, טבלת השלבים הספציפית לפרויקט נוצרה ללא העמודה `hierarchical_number` וללא עמודות חשובות אחרות כגון `dependencies`, `sort_order` ו-`parent_stage_id`.

### הפתרון

1. **עדכון פונקציית `create_project_stages_table`**:
   - ודאנו שהפונקציה כוללת את כל העמודות הדרושות, בפרט את `hierarchical_number`.
   - הוספנו מיגרציה חדשה: `20250507000000_fix_project_stages_table_function.sql`.

2. **יצירת פונקציית התיקון לטבלאות קיימות**:
   - שיפרנו את פונקציית `fix_project_stages_table` כך שתבדוק שהעמודה `hierarchical_number` והעמודות האחרות קיימות.
   - יצרנו פונקציה חדשה `fix_specific_project_stages_table` שתתקן טבלת שלבים ספציפית לפרויקט.

3. **עדכון שירות השלבים**:
   - עדכנו את `stageService.ts` להפעיל את פונקציית התיקון אם נתקלים בשגיאה בעת יצירת שלב חדש.
   - הוספנו טיפול שגיאות טוב יותר בפונקציה `createStage`.

4. **תיעוד**:
   - יצרנו קובץ הוראות התקנה `INSTALL.md` עם הוראות מפורטות להתקנת המיגרציות ותיקון טבלאות.
   - יצרנו קובץ README נוסף `README-fix-stages-tables.md` עם פתרונות מפורטים לבעיות בטבלאות שלבים.

### השפעה

- **פרויקטים חדשים**: ייצרו טבלאות עם כל העמודות הדרושות.
- **פרויקטים קיימים**: ניתן לתקן את הטבלאות באמצעות הפונקציות החדשות.
- **מניעת שגיאות**: הגדלנו את העמידות של המערכת בפני שגיאות דומות בעתיד.

### סיכום

שדרוג זה מבטיח שכל טבלאות השלבים הקיימות והעתידיות יכילו את כל העמודות הדרושות, ובכך מונע שגיאות בעת יצירת שלבים חדשים. בנוסף, הוספנו מנגנוני תיקון אוטומטיים שיכולים לתקן טבלאות פגומות במידת הצורך.
סיום תהליך פישוט פונקציות טבלאות שלבים - Thu Mar 27 21:15:22 IST 2025

## 2025-09-00 - פישוט פונקציות טיפול בטבלאות השלבים

נערך פישוט משמעותי של פונקציות SQL שמטפלות בטבלאות השלבים. המטרה: הפחתת כפילויות וייעול התחזוקה.

### שינויים שבוצעו:
- הוספה של פונקציה מאוחדת `manage_project_stages_table` שמחליפה שלוש פונקציות קודמות
- שמירה על פונקציות מעטפת לתאימות לאחור (`create_project_stages_table`, `fix_project_stages_table`, `fix_specific_project_stages_table`)
- שיפור בטיפול בשגיאות בתוך הפונקציות
- יצירת מנגנון אחיד לטיפול בעמודות חסרות בטבלאות

### קבצים שנוצרו:
- מיגרציה: `supabase/migrations/20250900000000_simplify_stage_functions.sql`
- הנחיות לשיפור קוד הקליינט: `simplify_client_code.md`
- סקריפט פישוט: `simplify_stages_functions.sh`

### צעדים הבאים:
- יישום השינויים המומלצים בקובץ `stageService.ts` לפי ההנחיות ב-`simplify_client_code.md`
- בדיקת תקינות המערכת עם הפונקציות החדשות

## 2025-10-15 - הוספת הוראות ידניות לפישוט פונקציות ניהול טבלאות שלבים

נוסף קובץ חדש `manual_simplify_instructions.md` המכיל הוראות מפורטות לביצוע ידני של השינויים הנדרשים בבסיס הנתונים.
הקובץ מציג את הצעדים הבאים:

1. העתקת תוכן הפונקציות החדשות מקובץ המיגרציה
2. הפעלת הקוד דרך ממשק ה-SQL של Supabase
3. וידוא שהפונקציות נוצרו בהצלחה
4. תיקון כל טבלאות השלבים הקיימות
5. רענון סכימת PostgREST
6. עדכון קוד ה-TypeScript בהתאם

קובץ זה מספק אלטרנטיבה למקרה שהסקריפט האוטומטי `simplify_stages_functions.sh` אינו פועל כמצופה.

המטרה היא לאפשר ליישם את השינויים בצורה ידנית דרך ממשק ה-Dashboard של Supabase, 
ללא תלות בכלי ה-CLI או בסקריפטים.

צעדים הבאים:
- ניתן ליישם את השינויים באמצעות אחת משתי הדרכים (סקריפט או ידני)
- לאחר היישום, יש לעדכן את קוד ה-TypeScript לפי ההנחיות ב-`simplify_client_code.md`
- לבדוק את תקינות המערכת עם הפונקציות החדשות

## 2025-11-01 - שיפור פונקציית createStage לטיפול טוב יותר בבעיות מבנה טבלה

ביצענו שיפורים בפונקציית createStage בשירות השלבים כדי להתמודד עם מקרים שבהם יש בעיה עם מבנה הטבלה וחסרות עמודות.

### שינויים שבוצעו:
- עדכון הטיפוס של פרמטר `stage` לטיפוס `any` כדי לאפשר גמישות בעבודה עם השדות השונים
- הוספת פונקציית עזר `getTableColumns` שבודקת אילו עמודות קיימות בטבלה
- יצירת מנגנון שמסנן את השדות לפי העמודות הקיימות בפועל בטבלה
- הוספת ניסיונות חלופיים ליצירת שלב עם שדות בסיסיים בלבד אם הניסיון הראשוני נכשל
- הוספת אפשרות ליצור את השלב בטבלה הרגילה 'stages' אם יש בעיה עם הטבלה הספציפית של הפרויקט

### השפעה:
- שיפור העמידות של המערכת בפני שגיאות הקשורות למבנה טבלאות
- יכולת טובה יותר להתאושש ממצבים שבהם חסרות עמודות בטבלה
- הפחתת שגיאות משתמש הקשורות לבעיות במבנה הטבלה

### צעדים הבאים:
- ניתן לשקול הוספת פונקציה דומה לשירות המשימות לטיפול בטבלאות משימות
- יש לבדוק את תקינות המערכת עם הפונקציה המשופרת בסביבות שונות

## 09.05.2025 - שיפור ממשק המשתמש להוספה ועריכה של משימות ושלבים

**שינויים:**
- שיפור משמעותי בממשק הוספת/עריכת משימות (TaskEditModal):
  - עיצוב מחדש עם כרטיסיות נושאיות והפרדה לוגית של השדות

## שינויים בדף יצירת פרויקט

### 28/03/2024 - הסרת לשונית "משימות ברירת מחדל" והשינויים הבאים:

1. הוסרה הלשונית "תבניות משימות ברירת מחדל"
2. שונה הפונקציה `init_project_tables_and_data` כך שלא תיצור משימות "משימה ראשונה" ו"משימה שנייה" אוטומטית
3. שונתה התנהגות פונקציות הסנכרון `syncProjectTasks` כך שלא תיצור משימות "משימה ראשונה" ו"משימה שנייה" אוטומטית
...
## 2025-05-10 - שיפור מיון משימות בדפי יצירת ועריכת משימות חדשות

- הורחב הפתרון למיון היררכי של משימות בכל הדפים הרלוונטיים:
  1. תוקנו דפי יצירת משימה ועריכת משימה:
     - הוספה שיטת מיון היררכית בפונקציה `fetchProjectTasks` בשני הדפים
     - משימות האב מוצגות כעת בסדר היררכי נכון (1, 1.1, 1.2, 2, וכו') במקום בסדר אלפביתי
     - נוספה בדיקה והגנה מפני ערכים לא תקינים בעת המיון

  2. מנגנון המיון כולל:
     - פיצול מספרים היררכיים לחלקים (שימוש ב-`split('.')` ומיפוי למספרים)
     - השוואה מספרית לפי כל חלק במספר
     - טיפול בשגיאות קצה והתמודדות עם ערכי `null`
     - מנגנון fallback למיון לפי כותרת אם המיון ההיררכי נכשל

המשימות בכל הרשימות הנפתחות מוצגות כעת בסדר היררכי עקבי ואינטואיטיבי.

- הוספת עמודת start_date ברשימת המשימות ובפורם העריכה עם איתחול אוטומטי לתאריך הנוכחי בעת יצירת משימה חדשה.
- כעת משתמשים יכולים לראות את תאריך ההתחלה של כל משימה ברשימה, ולערוך אותו בפורם המשימה.

## שיפור תצוגת רשימת משימות - תמיכה בתצוגת עץ

* שודרגה תצוגת רשימת המשימות כך שתהיה ברורה יותר ותציג היררכיה של משימות, תת-משימות ותת-תת-משימות
* נוספה אפשרות לחיצה על משימות שפותחת וסוגרת את תת-המשימות שלהן בצורת עץ
* הוסף עיצוב חזותי שמבהיר את היחסים בין המשימות (קווי חיבור, הזחה, רקע שונה)
* משימות בעלות רמות היררכיה שונות מקבלות עיצוב שונה שמאפשר להבדיל ביניהן בקלות
* משימות מוצגות במבנה עץ מדורג עם קווים המחברים אותן
