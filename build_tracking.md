...
- [x] תיקון שגיאות "null value in column id violates not-null constraint" בעת יצירת פרויקטים, שלבים ומשימות:
  - הוספת מזהה UUID לפרויקט חדש בעת יצירתו
  - הוספת מזהה UUID לכל שלב בעת יצירת שלבים ברירת מחדל
  - הוספת מזהה UUID לכל משימה בעת יצירת משימות ברירת מחדל
  - עדכון פונקציות createStage ו-createTask כדי לוודא שיש מזהה UUID
  - הוספת מזהה UUID לכל משימה משוכפלת בפונקציית cloneTasksToProject

- [x] שיפור דף יצירת פרויקט חדש:
  - הוספת אפשרות לבחירת משימות מותאמות אישית מתוך כל המשימות הקיימות
  - הוספת אפשרות לסינון משימות לפי תגיות
  - הוספת תצוגה של המשימות שנבחרו בצורת תגיות
  - שיפור ממשק המשתמש עם טאבים לבחירה בין משימות ברירת מחדל לבחירת משימות מותאמת
  - יצירת עותקים של המשימות הנבחרות ושיוכן לפרויקט החדש

### 2024-07-XX - יישום טבלאות ייחודיות לפרויקטים
#### מטרות השיפור:
- **שיפור ארגון הנתונים** - יצירת טבלה ייחודית לכל פרויקט במסד הנתונים
- **שיפור ביצועים** - הפרדת נתוני המשימות לטבלאות נפרדות לכל פרויקט
- **גמישות מבנית** - אפשרות להתאים את מבנה הנתונים לכל פרויקט בנפרד בעתיד

#### שינויים שבוצעו:
1. **עדכון פונקציות ניהול המשימות**:
   - ✅ `createTask` - עודכנה כך שתוסיף משימות גם לטבלה הייחודית של הפרויקט
   - ✅ `updateTask` - עודכנה כך שתעדכן משימות גם בטבלה הייחודית של הפרויקט
   - ✅ `deleteTask` - עודכנה כך שתמחק משימות גם מהטבלה הייחודית של הפרויקט
   - ✅ `getTasksByProject` - עודכנה כך שתקרא משימות מהטבלה הייחודית של הפרויקט
   - ✅ `cloneTasksToProject` - עודכנה כך שתוסיף משימות גם לטבלה הייחודית של הפרויקט
   - ✅ `createDefaultTasksForRealEstateProject` - עודכנה כך שתוסיף משימות גם לטבלה הייחודית של הפרויקט
   - ✅ הוספת פונקציה חדשה `getProjectSpecificTasks` לקריאת משימות מהטבלה הייחודית של הפרויקט

2. **תיקון שגיאות לינטר**:
   - ✅ עדכון הפונקציה `getNextRootHierarchicalNumber` כך שתוכל לקבל `project_id` שיכול להיות `null`
   - ✅ עדכון הפונקציה `addCustomTask` כך שתיצור משימות עם `project_id: null` במקום סטרינג ריק

3. **שיפור טיפול בשגיאות**:
   - ✅ הוספת טיפול בשגיאות בכל הפונקציות כדי להבטיח שהאפליקציה תמשיך לעבוד גם אם יש בעיות עם הטבלאות הייחודיות
   - ✅ הוספת בדיקות קיום של טבלאות לפני ניסיון לבצע פעולות עליהן
   - ✅ יישום מנגנון נפילה חזרה (fallback) לטבלה הראשית במקרה של שגיאה בטבלה הייחודית

4. **שיפורים נוספים**:
   - ✅ יישום מנגנון לבדיקת קיום טבלה ייחודית לפרויקט
   - ✅ יישום מנגנון ליצירת טבלה ייחודית לפרויקט אם היא לא קיימת
   - ✅ שמירת משימות במקביל בטבלה הראשית ובטבלה הייחודית של הפרויקט

#### יתרונות המימוש:
- **ביצועים משופרים** - שאילתות מהירות יותר לקבלת משימות של פרויקט ספציפי
- **בידוד נתונים** - הפרדה לוגית של נתוני המשימות לפי פרויקט
- **גמישות עתידית** - אפשרות להוסיף שדות או מבנים ייחודיים לפרויקטים ספציפיים
- **עמידות לשגיאות** - המערכת ממשיכה לעבוד גם אם יש בעיה עם הטבלה הייחודית
