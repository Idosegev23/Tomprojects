...
-- הוסף את אבני הדרך הקבועות לפרויקטים קיימים שאין להם אבני דרך
DO $$
DECLARE
...
BEGIN
    FOR project_record IN 
        ...
        WHERE id NOT IN (SELECT DISTINCT project_id FROM milestones)
    LOOP
        PERFORM create_default_milestones_for_project(project_record.id);
    END LOOP;
END $$; 